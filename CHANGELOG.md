...
### Новое
- **Режим медленной вставки** — флаг `-S` / `--slow` отправляет ввод в сессию посимвольно с задержкой, чтобы медленный консоль-сервер (Moxa с портом на 9600 и т.п.) не терял символы. Задержка между символами настраивается через `--char-delay <мс>` (по умолчанию 30). Замедляется весь набираемый ввод — команды, пароль внутри сессии (telnet/enable), пробел пагинации; SSH-аутентификация при этом не замедляется (пароль уходит на auth-уровне через askpass, а не по медленной линии). Прерывается по `Ctrl+C`/таймауту
